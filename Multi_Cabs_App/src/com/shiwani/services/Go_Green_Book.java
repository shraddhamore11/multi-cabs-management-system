...
/**
 * Servlet implementation class Go_Green_Book
 */
public class Go_Green_Book extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public Go_Green_Book() {
        super();
...
try
		
		{
			...
			PreparedStatement ps2=con.prepareStatement("select Go_Green_Bus from adminroute_tbl where Pick_Up_Point=? and Destination=?");
			ps2.setString(1,Pick_Up_Point);
			ps2.setString(2,Destination);
			ResultSet rs=ps2.executeQuery();
			while(rs.next())
			{
			Rate  = rs.getString("Go_Green_Bus");
			}
			...
			{
				response.sendRedirect("Go-Green_Booking.html");
			}
		}
		catch(Exception e)
		{
