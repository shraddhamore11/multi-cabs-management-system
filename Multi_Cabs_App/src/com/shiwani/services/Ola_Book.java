...
/**
 * Servlet implementation class Ola_Book
 */
public class Ola_Book extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		
		{
			...
			PreparedStatement ps2=con.prepareStatement("select Ola_Rate from adminroute_tbl where Pick_Up_Point=? and Destination=?");
			ps2.setString(1,Pick_Up_Point);
			ps2.setString(2,Destination);
			ResultSet rs=ps2.executeQuery();
			while(rs.next())
			{
			Rate  = rs.getString("Ola_Rate");
			}
			...
			{
				response.sendRedirect("Ola_Booking.html");
			}
		}
		catch(Exception e)
		{
