...
/**
 * Servlet implementation class Uber_Book
 */
public class Uber_Book extends HttpServlet {
...
try
		
		{
			...
			PreparedStatement ps2=con.prepareStatement("select Uber_Rate from adminroute_tbl where Pick_Up_Point=? and Destination=?");
			ps2.setString(1,Pick_Up_Point);
			ps2.setString(2,Destination);
			ResultSet rs=ps2.executeQuery();
			while(rs.next())
			{
			Rate  = rs.getString("Uber_Rate");
			}
			...
			{
				response.sendRedirect("Uber_Booking.html");
			}
		}
		catch(Exception e)
		{
